...
\begin{abstract}
This report presents a computational framework for analyzing calendar-spread dynamics in CME copper (HG) futures from 2009--2024, based on minute-level data aggregated to hourly periods. Using deterministic expiry-based contract labeling with hour-precision timing, we detect 711 S1 (front spread) widening events representing 1.60\% of all hourly periods. Multi-spread analysis (S1--S11) reveals systematic event patterns across near-month spreads (S1--S5), with S2 showing the highest detection rate (3.99\%, 1,774 events) despite being one contract further from expiry. The median S1 event occurs 20.6 days before F1 expiry (IQR: 14.7--26.0 days), well within the institutional rolling window. Strip diagnostics classify 44.1\% of trading days as expiry-dominated based on S1 magnitude exceeding 2$\times$ the median of S2--S11. Volume-based liquidity confirmation shows 84.6\% of daily widening events (132 of 156) coincide with volume migration from F1 to F2, providing strong independent validation. Intraday distribution shows concentration in US market hours (09:00--15:00 CT) with 77 events (50\% share), alongside significant overnight activity in Asian (27 events, 17.5\%) and European (29 events, 18.8\%) sessions. The framework implements strict calendar-only business day calculations with comprehensive DST handling, ensuring temporal precision throughout the analysis pipeline.
\end{abstract}

\vspace{1em}
\noindent\textbf{Version Note (v2.0, November 2024)}: This report documents the analysis framework as of version 2.0.0, which introduced deterministic expiry-based contract labeling with exact hour-precision switching at documented expiry instants (e.g., 17:00 CT for copper contracts). The methodology supersedes earlier approaches that depended on data availability for contract identification. All results presented reflect the v2.0.0 implementation, ensuring F2 becomes F1 at the precise expiry timestamp rather than at midnight boundaries or when price data first appears. The trading calendar is now a strict requirement with no weekday fallback, and all timing calculations use hour precision internally while reporting in days for readability. The legacy \texttt{selection.mode} configuration option has been removed; expiry-based labeling is now the only supported method. \textbf{Note on filtering}: Unlike earlier versions, expiry-dominance filtering is no longer applied to raw event counts reported in this document. All statistics reflect unfiltered detection results unless explicitly stated otherwise. Strip diagnostics (Section 4.2) provide classification information but do not filter event counts.
...
Applying z-score detection (threshold > 1.5, 20-period window, 3-hour cooldown) to 44,419 hourly periods yields:

\begin{table}[H]
\centering
\caption{S1 (Front Spread) Event Detection Results}
\label{tab:filter}
\small
\begin{tabular}{lrrrr}
\toprule
...
\midrule
Raw S1 events & 711 & 1.60\% & 20.6 & 14.7--26.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
    \item \textbf{1.60\% detection rate}: Approximately 1 in 62 hourly periods triggers an S1 widening event
    \item \textbf{Median timing of 20.6 days}: Events cluster within the institutional rolling window (typically 15--30 days before expiry)
    \item \textbf{Consistent IQR (14.7--26.0 days)}: Indicates systematic timing patterns rather than random noise
\end{itemize}

\textbf{Note on filtering}: Unlike earlier framework versions, no expiry-dominance filtering is applied to event counts. The 711 events represent raw detection results. Strip diagnostics (Section 4.1) classify 557 of these events as expiry-dominated, but this classification does not filter the reported counts.

\subsection{Intraday Event Distribution}

Events concentrate in US market hours with significant overnight activity (Table~\ref{tab:buckets}). The US Open bucket (09:00 CT) shows the highest single-bucket concentration (22.1\%), indicating positioning adjustments at market open.
...
\midrule
1 & 09:00 -- US Open & US Regular & 34 & 22.1 \\
2 & 10:00 -- US Morning & US Regular & 17 & 11.0 \\
3 & 11:00 -- US Late Morning & US Regular & 10 & 6.5 \\
4 & 12:00 -- US Midday & US Regular & 9 & 5.8 \\
5 & 13:00 -- US Early Afternoon & US Regular & 6 & 3.9 \\
6 & 14:00 -- US Late Afternoon & US Regular & 3 & 1.9 \\
7 & 15:00 -- US Close & US Regular & 1 & 0.6 \\
8 & Late US / After-Hours & Late US & 18 & 11.7 \\
9 & Asia Session & Asia & 27 & 17.5 \\
10 & Europe Session & Europe & 29 & 18.8 \\
\midrule
\textbf{Total} & & & 154 & 100.0 \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
...
\begin{itemize}[noitemsep]
    \item \textbf{US Regular Hours} (Buckets 1--7): 77 events (50.0\%)
    \item \textbf{Late US} (Bucket 8): 18 events (11.7\%)
    \item \textbf{Asia} (Bucket 9): 27 events (17.5\%)
    \item \textbf{Europe} (Bucket 10): 29 events (18.8\%)
\end{itemize}

\textbf{Interpretation}: US market hours dominate with 50\% share, but significant overnight activity (38.0\% combined Asia + Europe) suggests global institutional participation in copper rolling.
...
\midrule
Spread widening events & 156 & 132 & 84.6 \\
Liquidity roll triggers & 2,382 & 132 & 5.5 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: 132 of 156 widening events (84.6\%) coincide with volume migration from F1 to F2.

\textbf{Interpretation}: The 84.6\% overlap rate provides strong independent confirmation that detected spread-widening episodes capture meaningful position transfers rather than calendar noise or statistical artifacts. The high alignment suggests institutional rolling activity leaves dual footprints in both spread dynamics and volume patterns.
...
\begin{table}[H]
\centering
\caption{Event Detection Across Calendar Spreads}
\begin{tabular}{lrrr}
\toprule
\textbf{Spread} & \textbf{Events} & \textbf{Rate (\%)} & \textbf{Median Days to F1 Expiry} \\
\midrule
S1 (F2-F1) & 711 & 1.60 & 20.6 \\
S2 (F3-F2) & 1,774 & 3.99 & 14.6 \\
S3 (F4-F3) & 874 & 1.97 & 12.3 \\
S4 (F5-F4) & 248 & 0.56 & 11.3 \\
S5 (F6-F5) & 19 & 0.04 & 4.8 \\
\midrule
S6--S11 (all) & 0 & 0.00 & -- \\
\bottomrule
\end{tabular}
...
\begin{itemize}[noitemsep]
    \item \textbf{S2 shows highest event rate}: 3.99\% detection rate (1,774 events) substantially exceeds S1 (1.60\%), demonstrating that elevated spread widening is \emph{not} unique to the front spread
    \item \textbf{S3 comparable to S1}: 1.97\% rate (874 events) indicates systematic activity extends deep into the curve
    \item \textbf{S6--S11 produce zero events}: Current detection parameters (z > 1.5) are insufficiently sensitive to detect far-dated spread movements, or far spreads exhibit fundamentally lower volatility
\end{itemize}
...
\texttt{roll\_signals/} & \texttt{hourly\_spread.csv} & Calendar spread series (S1) \\
 & \texttt{hourly\_widening.csv} & Event timestamps (711 events) \\
...
\texttt{roll\_signals/} & \texttt{hg\_spread\_filtered.csv} & Daily spread series \\
 & \texttt{hg\_widening\_filtered.csv} & Daily widening events (156) \\
